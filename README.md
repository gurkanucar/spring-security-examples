# spring-security-examples

### Examples are in different branches

[in-memory-auth](https://github.com/gurkanucar/spring-security-examples/tree/in-memory-auth)

[basic-auth-hardcoded-one-role](https://github.com/gurkanucar/spring-security-examples/tree/basic-auth-hardcoded-one-role)

[basic-auth-hardcoded-multiple-roles](https://github.com/gurkanucar/spring-security-examples/tree/basic-auth-hardcoded-multiple-roles)

[basic-auth-dynamic-roles](https://github.com/gurkanucar/spring-security-examples/tree/basic-auth-dynamic-roles)

[basic-auth-role-based-voter (dynamic endpoint restriction by role)](https://github.com/gurkanucar/spring-security-examples/tree/basic-auth-role-based-voter)

[jwt-very-basic](https://github.com/gurkanucar/spring-security-examples/tree/jwt-very-basic)

[jwt-with-refresh-token](https://github.com/gurkanucar/spring-security-examples/tree/jwt-with-refresh-token)

[jwt-and-otp](https://github.com/gurkanucar/spring-security-examples/tree/jwt-and-otp)
